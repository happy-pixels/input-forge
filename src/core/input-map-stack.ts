...
export class InputMapStack {
    ...
    private validateInputMap(inputMap: InputMap, method: string): boolean {
    ...
    public getCommandsForSingleInput(key: string): Command[] {
        return this.getOrResolve(
            key,
            this.singleInputCache,
            (map) => this.resolver.resolveSingleInput(map, key)
        );
    }
    ...
    public getCommandsForAxesInput(name: string): AxesCommand[] {
        return this.getOrResolve(
            name,
            this.axesInputCache,
            (map) => this.resolver.resolveAxesInput(map, name)
        );
    }
    ...
    public getAxesCommandsForKey(key: string): Array<{ command: AxesCommand; axis: AxesInput }> {
        return this.getOrResolve(
            key,
            this.keyboardAxesCache,
            (map) => this.resolver.resolveKeyboardAxesInput(map, key)
        );
    }
    ...
    public getTickCommands(): TickCommand[] {
        ...
        const currentMap = this.getCurrent() as InputMap;
